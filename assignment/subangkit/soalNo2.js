// Nama : Subangkit
// FE_22

// Seorang siswa bernama Gabriel telah menyelesaikan studi nya selama 5 semester, 
// dimana masing-masing semester dia akan menerima 1 nilai akhir. 
// Hitung rata-rata dari 5 semester tersebut, 
// kemudian tentukan Grade yang didapatkan Gabriel dengan rules yang telah ditentukan.

// Berikut adalah nilai akhir dari Gabriel untuk masing-masing semesternya:
// Semester 1 : 85
// Semester 2 : 95.20
// Semester 3 : 67.75
// Semester 4 : 78
// Semester 5 : 75

// Berikut adalah rules untuk menentukan Grade dari nilai Gabriel
// A : rata-rata nilai lebih besar sama dengan 90 DAN kurang dari sama dengan 100
// B : rata-rata nilai lebih besar sama dengan 80 DAN kurang dari 90
// C : rata-rata nilai lebih besar sama dengan 75 DAN kurang dari 80
// D : rata-rata nilai lebih besar sama dengan 60 DAN kurang dari 75
// E : rata-rata nilai kurang dari 60

const namaMahaSiswa = 'Gabriel';
const nilaiSemester1 = 85;
const nilaiSemester2 = 95.20;
const nilaiSemester3 = 67.75;
const nilaiSemester4 = 78;
const nilaiSemester5 = 75;

let nilaiAkhir = 0;

const nilaiRataRata = (nilaiSemester1 + nilaiSemester2 + nilaiSemester3 + nilaiSemester4 + nilaiSemester5) / 5;

nilaiAkhir = nilaiRataRata;

console.log("Nilai akhir dari Gabriel adalah =", nilaiAkhir.toFixed(2));


if (nilaiAkhir < 0 || nilaiAkhir > 100){
    console.error ("Maaf, Nilai yang dimasukkan tidak valid");
}else {
    if(nilaiAkhir >= 90 && nilaiAkhir <= 100) {
    console.log(`${namaMahaSiswa} mendapatkan Grade A`);
}else if (nilaiAkhir >= 80 && nilaiAkhir < 90){
    console.log(`${namaMahaSiswa} mendapatkan Grade B`);
}else if (nilaiAkhir >= 75 && nilaiAkhir < 80){
    console.log(`${namaMahaSiswa} mendapatkan Grade C`);
}else if (nilaiAkhir >= 60 && nilaiAkhir < 75){
    console.log(`${namaMahaSiswa} mendapatkan Grade D`);
}else{
    console.log(`mendapatkan Grade E`);
}
}